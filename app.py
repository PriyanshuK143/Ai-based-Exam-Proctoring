...
def tearegister():
    ...
    if request.method == "POST":
        ...
        hashed_passwordtea = generate_password_hash(passwordtea, method='pbkdf2:sha256')
...
def generate_frames(camera,detector,count):
    ...
    def detect_mouth_openness(frame):
    ...
    # Iterate through each detected face
        ...
        # Extract mouth landmarks
            mouth_points = [(landmarks.part(i).x, landmarks.part(i).y) for i in mouth_landmarks]
        
        # Convert mouth points to a NumPy array
            mouth_points = np.array(mouth_points)
        
        # Draw the lip shape
            lip_shape = cv2.convexHull(mouth_points)
            cv2.drawContours(frame, [lip_shape], -1, (0, 255, 0), 2)
        
        # Calculate the change in mouth width (e.g., sum of horizontal distances between points)
            mouth_width = sum(mouth_points[i][0] for i in range(len(mouth_points))) - len(mouth_points) * mouth_points[0][0]
        
        # Define a threshold for mouth openness (adjust this threshold as needed)
            threshold = 500
        
        # Check if the mouth is open based on the threshold
            if mouth_width > threshold:
                return "Mouth open" 
            else:
                return "Mouth closed" 
    ...
    while True:
        ...
        if not success:
            break
                ...
                gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
                faces = detector(gray)
                mouth_state = detect_mouth_openness(frame)

                # Display the mouth state on the frame
                cv2.putText(frame, mouth_state, (50, 450), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
